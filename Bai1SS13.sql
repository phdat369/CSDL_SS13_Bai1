...
FOR EACH ROW
BEGIN
-- Lôi logc: Đang lây ngày cũ ra so sánh với hiện tại thay vì kiểm tra ngày mới
IF OLD.appointment_date < NOW( ) THEN
SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Lôi: Không the đặt lịch khám vào thời điem trong quá';
END IF;
END //
DELIMITER ;

update Appointments 
set appointment_date = '2026-05-05'
where appointment_id = 104;
-- Hiện tại logic đang sai ở trong bài là do đang lấy ngày cũ so sánh với ngày hiện tại, đúng ra là phải lấy dữ liệu sau khi cập nhật, thì chúng ta phải dùng new thay vì dùng old vì thế gây ra lỗi logic trên 
...
-- check 
update Appointments 
set appointment_date = '2026-05-04'
where appointment_id = 104;